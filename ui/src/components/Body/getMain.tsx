...
export function GetMain(
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {(uploadStatus && !fileUrls) && <CircularProgress sx={{position: 'absolute', top: "50%", left: "50%", transform: 'translate(-50%, -50%)'}}/>}
      {(submitted && !outputUrl) && <CircularProgress sx={{position: 'absolute', top: "50%", left: "50%", transform: 'translate(-50%, -50%)'}}/>}
      {(fileUrls && !submitted) &&
      ...
      {fileUrls && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}><Scene spatialParams={spatialParams} fileLabels={fileLabels} /></div>}
      {outputUrl && <div style={{ position: 'absolute', bottom: 10, left: 10 }}><MemoAudioFilePlayer audioURL={outputUrl}/></div>}
    </div>
  );
}